...
export default function CriticalPathPage() {
  ...
  const handleShiftTask = async (taskId: string, direction: 'left' | 'right') => {
    setTasks(prevTasks =>
      prevTasks.map(t => {
        if (t.id !== taskId) return t;

        const newStart = direction === 'left' ? Math.max(1, t.startDay - 1) : Math.min(14, t.startDay + 1);
        ...
        supabase
          ...
          .eq('id', taskId)
          .then(({ error }) => {
            if (error) console.error('Error al guardar el desplazamiento:', error);
          });
  ...
  const filteredTasks = tasks.filter(t => {
    const matchesProject = selectedProject === 'Todos los Proyectos' || t.project.trim().toLowerCase() === selectedProject.trim().toLowerCase();
    const matchesCritical = onlyCritical ? t.isCritical : true;
    
    let matchesStatus = true;
    if (statusFilter === 'Completadas') matchesStatus = t.status === 'Completada';
    if (statusFilter === 'En Proceso') matchesStatus = t.status === 'En Proceso';
    if (statusFilter === 'Pendientes') matchesStatus = t.status === 'Pendiente';
    if (statusFilter === 'Postergadas') matchesStatus = t.status === 'Postergada';
      ...
      <main className="flex-1 flex flex-col p-5 md:p-6 overflow-hidden h-full min-w-0">
        ...
        <header className="flex justify-between items-center mb-4 shrink-0">
          <div>
            <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              GM Diagrama Gantt Desplazable & Ruta Crítica
            </h1>
            <p className="text-xs text-slate-500">
              Desplaza actividades con ◄ / ► para simular postergaciones autorizadas por el Administrador.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4 shrink-0">
          <div className="bg-white p-3 rounded-2xl border border-slate-200/80 shadow-2xs flex items-center justify-between">
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Duración Ruta Crítica</p>
          ...
          <div className="bg-white p-3 rounded-2xl border border-rose-100 shadow-2xs flex items-center justify-between">
            <div>
              <p className="text-[10px] font-bold text-rose-500 uppercase tracking-wider">Actividades Críticas</p>
              <h3 className="text-base font-bold text-rose-600">{criticalCount} Sin Holgura</h3>
            </div>
          ...
          <div className="bg-white p-3 rounded-2xl border border-indigo-100 shadow-2xs flex items-center justify-between">
            <div>
              <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-wider">Marcador de Avance</p>
              <h3 className="text-base font-bold text-indigo-700">Día {CURRENT_DAY_MARKER} Activo</h3>
            </div>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  <select
                    ...
                    className="bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-1 text-xs font-bold text-slate-900 outline-none focus:ring-2 focus:ring-blue-500/20 cursor-pointer"
                  >
                    {projects.map((proj) => (
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold text-slate-800 uppercase tracking-wider">Estado:</span>
                  <select
                    value={statusFilter}
                  ...
                  >
                    ...
                    <option value="Postergadas">⚠️ Postergadas / Reagendadas</option>
                  </select>
                </div>
              </div>

              {/* Leyenda Explicativa */}
              <div className="flex items-center gap-2.5 text-[10px] font-bold bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200/80 flex-wrap">
                <span className="flex items-center gap-1 text-emerald-600">
                  <span className="w-2.5 h-2.5 rounded-sm bg-emerald-500"></span> ✓ Completada
                </span>
                <span className="flex items-center gap-1 text-amber-600">
                  <span className="w-2.5 h-2.5 rounded-sm bg-amber-500"></span> ⚠️ Postergada
                </span>
                <span className="flex items-center gap-1 text-rose-600">
                  <span className="w-2.5 h-2.5 rounded-sm bg-rose-500 animate-pulse"></span> 🔴 Ruta Crítica
                </span>
              </div>
            </div>
                  ...
                  <div className="space-y-2.5 relative">
                    {filteredTasks.map((task) => {
                      ...
                      return (
                        <div 
                          key={task.id} 
                          className={`grid grid-cols-12 gap-1 items-center p-2.5 rounded-xl transition-all border group ${
                            task.isMilestone
                          ...
                          <div className="col-span-4 pr-2 space-y-1">
                            ...
                            <div className="flex items-center justify-between text-[10px] text-slate-500 font-medium">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="truncate max-w-[85px]">{task.assigneeName}</span>
                              </div>
                          ...
                          >
                            {task.isMilestone ? (
                              <div
                                ...
                                className="h-full flex items-center gap-1 text-amber-800 font-bold text-[10px] bg-amber-200 border border-amber-400 rounded-lg px-2 shadow-2xs"
                              >
                                <span>◆</span>
                              ...
                              <div
                                ...
                                className={`h-full rounded-lg flex items-center justify-between px-2 text-[10px] font-bold text-white shadow-xs transition-all relative overflow-hidden cursor-pointer ${
                                  task.isCritical 
                              ...
                              >
                                ...
                                <span className="relative z-10 truncate flex items-center gap-1 drop-shadow-xs">
                                  ...
                                  {isPostponed && <span>⏱️</span>}
                                  Duración: {duration}d
                                </span>
                                <span className="relative z-10 font-mono bg-black/40 px-1.5 py-0.5 rounded text-[9px]">
                                  {progress}%
                                </span>
                              </div>
                            )}
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-100 p-6 space-y-4">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <div>
                <h3 className="text-sm font-bold text-slate-900">Editar Actividad del Gantt</h3>
                <p className="text-[11px] text-slate-500">ID: {editingTask.id} • {editingTask.project}</p>
              </div>
              <button 
            ...
            <form onSubmit={handleSaveTaskEdit} className="space-y-3.5 text-xs">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Inicio (Día)</label>
                  <input 
                    type="number" 
                    min="1" 
                    max="15" 
                    ...
                    onChange={(e) => setEditingTask({ ...editingTask, startDay: parseInt(e.target.value) || 1 })} 
                    className="w-full border border-slate-300 rounded-xl p-2.5 text-slate-900 font-medium bg-white outline-none" 
                  />
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Duración (Días)</label>
                  <input 
                    type="number" 
                    min="1" 
                    required 
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Holgura (Días)</label>
                  <input 
                    type="number" 
                    min="0" 
                    required 
                    value={editingTask.slackDays} 
                    onChange={(e) => setEditingTask({ ...editingTask, slackDays: parseInt(e.target.value) || 0 })} 
                    className="w-full border border-slate-300 rounded-xl p-2.5 text-slate-900 font-medium bg-white outline-none" 
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-600 uppercase mb-1">Estado de la Tarea</label>
                <select
                  value={editingTask.status}
                  onChange={(e) => {
                    const newStatus = e.target.value as 'Pendiente' | 'En Proceso' | 'Completada' | 'Postergada';
                    ...
                    if (newStatus === 'En Proceso' && (autoProgress === 0 || autoProgress === 100)) autoProgress = 50;

                    setEditingTask({ ...editingTask, status: newStatus, progressPercent: autoProgress });
                  }}
                  className="w-full border border-slate-300 rounded-xl p-2.5 text-slate-900 font-medium bg-white outline-none"
                >
                  <option value="Pendiente">⏳ Pendiente (0%)</option>
                  <option value="En Proceso">🔵 En Proceso</option>
                  <option value="Postergada">⚠️ Postergada / Reagendada (Tiempo Extra)</option>
                ...
                <div className="flex gap-1.5">
                  {[0, 25, 50, 75, 100].map((p) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        let newStatus: 'Pendiente' | 'En Proceso' | 'Completada' | 'Postergada' = editingTask.status;
                        if (p === 0) newStatus = 'Pendiente';
                        if (p === 100) newStatus = 'Completada';

                        setEditingTask({ ...editingTask, progressPercent: p, status: newStatus });
                      }}
                      className={`flex-1 py-1.5 rounded-xl font-bold text-xs transition-all cursor-pointer border ${
              ...
              <div className="flex gap-4 pt-1">
                <label className="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer">
                  <input 
                    ...
                    onChange={(e) => setEditingTask({ ...editingTask, isCritical: e.target.checked })} 
                    className="rounded text-rose-600 focus:ring-rose-500 cursor-pointer" 
                  />
                  <span>🔴 Pertenece a Ruta Crítica</span>
                </label>
