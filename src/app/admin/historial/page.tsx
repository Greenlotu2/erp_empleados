...
const STATUSES = [
  ...
  { id: "Postergada", name: "Postergadas / Reagendadas" },
];

export default function HistorialPage() {
  const [tasksHistory, setTasksHistory] = useState<TaskHistoryItem[]>([]);
  const [employeesList, setEmployeesList] = useState<FilterOption[]>([
    { id: "all", name: "Todos los Empleados" },
  ]);
  const [projectsList, setProjectsList] = useState<FilterOption[]>([
  ...
  const fetchHistorialData = async () => {
    try {
      ...
      const empMap = new Map(
        (empData || []).map((e: any) => [String(e.id), e.nombre]),
      );
      const projMap = new Map(
        (projData || []).map((p: any) => [String(p.id), p.nombre]),
      );
  ...
  const filteredHistory = useMemo(() => {
    return tasksHistory.filter((task) => {
      const matchesEmployee =
        selectedEmployee === "all" || task.employee === selectedEmployee;
      ...
      return (
        matchesEmployee && matchesProject && matchesStatus && matchesSearch
      );
    });
  }, [
    tasksHistory,
  ...
  const delayedCount = filteredHistory.filter(
    (t) => t.status === "Postergada",
  ).length;
  ...
  if (loading) {
    return (
      <div className="h-screen w-screen bg-slate-50 flex items-center justify-center font-sans p-4">
        <div className="flex items-center gap-2.5 bg-white border border-slate-200 rounded-xl shadow-sm px-3.5 py-2.5">
  ...
  if (errorMessage) {
    return (
      <div className="h-screen w-screen bg-slate-50 flex items-center justify-center font-sans p-2">
  ...
  return (
    <div className="h-screen w-screen bg-slate-50 flex font-sans overflow-hidden select-none">
      ...
      <main className="flex-1 flex flex-col p-3 overflow-hidden h-full min-w-0">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-1.5 mb-2 shrink-0">
          <div>
            <h1 className="text-[15px] font-semibold text-slate-900 tracking-tight flex items-center gap-1.5">
              <Icon name="scroll" size={16} className="text-slate-400" />{" "}
              Historial de Actividades
            </h1>
            <p className="text-xs text-slate-500">
              Registro cronológico de tareas asignadas, tiempos de ejecución y
          ...
          <div className="flex flex-wrap items-center gap-1 w-full lg:w-auto">
            ...
            <div className="relative flex-1 sm:w-48">
              <input
                ...
                className="w-full bg-white border border-slate-200 text-slate-700 text-xs font-medium py-1.5 pl-7 pr-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 shadow-2xs"
              />
              <Icon
                name="search"
                size={13}
            ...
            <select
              ...
              className="bg-white border border-slate-200 text-slate-700 text-xs font-semibold py-1 px-1.5 rounded-xl focus:outline-none cursor-pointer shadow-2xs"
            >
              {STATUSES.map((st) => (
        ...
        <div className="bg-white border border-slate-200/80 rounded-lg p-2 mb-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-1.5 shrink-0">
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            ...
            <div className="flex flex-wrap items-center gap-1 text-xs">
              ...
              <span className="bg-emerald-50 text-emerald-700 font-bold px-1 py-0.5 rounded-md border border-emerald-200/60">
                {completedCount} completadas
              </span>

              {delayedCount > 0 && (
                <span className="bg-rose-50 text-rose-700 font-bold px-1 py-0.5 rounded-md border border-rose-200/60">
                  {delayedCount} postergadas
                </span>
              )}

              {pendingCount > 0 && (
                <span className="bg-amber-50 text-amber-700 font-bold px-1 py-0.5 rounded-md border border-amber-200/60">
          ...
          <div className="flex items-center gap-1 w-full sm:w-auto">
            <button
              onClick={fetchHistorialData}
              className="bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 font-bold py-1 px-1.5 rounded-xl text-xs transition-colors flex items-center justify-center gap-1 cursor-pointer"
            >
              <Icon name="refresh" size={13} /> Recargar
            </button>
            <button
              onClick={() => alert("Generando PDF...")}
              className="flex-1 sm:flex-initial bg-red-50 hover:bg-red-100 border border-red-200 text-red-700 font-bold py-1 px-1.5 rounded-xl text-xs transition-colors flex items-center justify-center gap-1 cursor-pointer"
            >
              <Icon name="download" size={13} /> Exportar PDF
            </button>
            <button
              onClick={() => alert("Exportando a Excel...")}
              className="flex-1 sm:flex-initial bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-700 font-bold py-1 px-1.5 rounded-xl text-xs transition-colors flex items-center justify-center gap-1 cursor-pointer"
            >
        ...
        {/* TABLA DE HISTORIAL */}
        <div className="flex-1 bg-white border border-slate-200/80 rounded-lg flex flex-col min-h-0 overflow-hidden">
            ...
            <table className="w-full text-left table-fixed min-w-[900px]">
              <thead className="sticky top-0 bg-slate-50/90 backdrop-blur-xs z-10 border-b border-slate-200">
                <tr className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                  <th className="py-2 px-2 w-[18%]">Responsable</th>
                  <th className="py-2 px-2 w-[20%]">Proyecto / Módulo</th>
                  <th className="py-2 px-2 w-[30%]">Descripción de la tarea</th>
                  <th className="py-2 px-2 w-[10%]">Inicio</th>
                  <th className="py-2 px-2 w-[10%]">Límite</th>
                ...
                {filteredHistory.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-6 text-center text-slate-400">
                      No se encontraron actividades registradas con los filtros
                      seleccionados.
                    </td>
                  </tr>
                ) : (
                  filteredHistory.map((task) => (
                    ...
                    >
                      {/* RESPONSABLE */}
                      <td className="py-2 px-2 font-semibold text-slate-800 truncate">
                      ...
                      <td className="py-2 px-2">
                        <span className="inline-flex items-center gap-1 px-1.5 py-1 rounded-lg text-[11px] font-semibold bg-slate-100 text-slate-700 border border-slate-200 truncate max-w-full">
                          {task.project}
                        </span>
                      </td>

                      {/* DESCRIPCIÓN */}
                      <td className="py-2 px-2 break-words pr-2 text-slate-600 font-normal leading-relaxed">
                        {task.task}
                      </td>

                      {/* FECHA INICIO */}
                      <td className="py-2 px-2 font-mono text-[11px] text-slate-500 font-medium">
                        {task.startDate}
                      </td>
                      ...
                      <td className="py-2 px-2 text-center">
                        <span
                          className={`px-1.5 py-1 rounded-full text-[10px] font-bold border inline-flex items-center gap-1 justify-center min-w-[95px] ${
                        ...
                        >
                          ...
                          {task.status === "Postergada" &&
                            `Postergada ${task.delayedDays ? `(+${task.delayedDays}d)` : ""}`}
