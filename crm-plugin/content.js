(function() {
  ...
  // Helper: envía la petición al background service worker en vez de hacer fetch directo,
  // así evitamos el bloqueo de mixed content / CORS al inyectarse en páginas HTTPS.
  ...
  function renderLoginScreen() {
    ...
    // --- PETICIÓN HTTP POST AL SERVIDOR (vía background) ---
    btnLogin.onclick = async () => {
      ...
      const result = await apiRequest('http://localhost:3000/api/auth/login', {
        method: 'POST',
      ...
      if (result.ok) {
        usuarioAutenticado = result.data.user; // Guardamos el id, name y role devuelto por la API vinculada a Supabase
        ...
        alert(result.data?.message || 'Credenciales incorrectas. Intenta de nuevo.');
        btnLogin.innerText = 'Conectar';
        btnLogin.disabled = false;
      }
    };
  ...
  async function renderDashboardScreen() {
    ...
    // --- PETICIÓN HTTP GET FILTRADA POR EMPLEADO ID (vía background) ---
    const result = await apiRequest(`http://localhost:3000/api/tareas?empleadoId=${usuarioAutenticado.id}`, { method: 'GET' });
    ...
    tareas.forEach(tarea => {
      ...
      // --- ACCIÓN REAL: MODIFICAR ESTADO (vía background) ---
